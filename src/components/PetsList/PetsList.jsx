		...
		<>
			{pets.map(({ _id, avatarURL, name, date, breed, comments }) => (
				<Box key={_id}>
					...
					{isModalOpen && <WarningMessage type="approve" id={_id} approveFunk={deletePetItem} onClose={closeModal} text="Are you sure than you want to delete the pet?" title="Approve" />}
				</Box>
			))}
		</>
	);
